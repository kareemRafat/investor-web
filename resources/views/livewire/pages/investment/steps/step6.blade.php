    ...
    <div class="step_height bg-white rounded-8 shadow-sm p-3 p-md-3 p-lg-4">
        <div class="row g-3">
            ...
            <div class="col-12 position-relative mt-4" dir="ltr">
                ...
                <input type="file" id="investor-attachment" class="d-none" wire:model="state.step6.data.attachment">
                {{-- Display selected or current file name --}}
                @if ($state['step6']['data']['attachment'] || $state['step6']['currentAttachment'] !== 'Uploaded File')
                    <div class="mt-2 d-flex align-items-center gap-2"
                        dir="{{ app()->getLocale() == 'en' ? 'ltr' : 'rtl' }}">
                        <small class="text-primary fw-bold">
                            {{ __('investor.steps.step6.selected_file') }}:
                            <span
                                class="mx-2">{{ $state['step6']['data']['attachment'] ? $state['step6']['data']['attachment']->getClientOriginalName() : $state['step6']['currentAttachment'] }}</span>
                        </small>
                    </div>
                @endif
            </div>
