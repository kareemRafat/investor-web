<div class="container px-sm-0">
    <div class="row g-3 mb-3">
        <div class="col-12">
            <div
                class="bg-light text-dark rounded-8 shadow-sm mb-3 d-flex justify-content-center gap-0 gap-md-3 gap-lg-4 flex-wrap">
                <h5 class="mb-0 p-3 fw-bold text-center">
                    ملخص فكرتك المميزة
                </h5>
            </div>
            <div class="d-flex flex-column gap-3">
                <div class="bg-white rounded-8 shadow-sm p-3 p-md-3 p-lg-4 pb-5">
                    <div class="mb-2">
                        <strong>السيد (الاسم المستعار):</strong>
                    </div>
                    <h6 class="fw-bold mb-0">
                        لقد تم إدراج فكرتك المميزة في صندوق الأفكار، نتمنى أن تجد الطرف المناسب لتنفيذها وفق
                        متطلباتك.
                    </h6>
                </div>

                <div class="bg-white rounded-8 shadow-sm p-3 p-md-3 p-lg-4 pb-5">
                    <h6 class="fw-bold mb-0">
                        القائمة أدناه تحتوي على العروض الاستثمارية المتوافقة / القريبة من متطلبات تنفيذ فكرتك،
                        نأمل
                        أن تجد المناسب منها:
                    </h6>
                </div>

                <div class="row mx-0 px-0 g-2">
                    <!-- Loop Here! -->
                    <div class="col-12">
                        <div class="card border-0 shadow-sm rounded-8 position-relative">
                            <!-- index -->
                            <div class="bg-custom position-absolute top-0 end-0 bg_idea_num rounded-circle m-1">
                                <div class="d-flex align-items-center justify-content-center h-100">
                                    <span class="text-white fw-bold fs-6">
                                        1
                                    </span>
                                </div>
                            </div>
                            <div class="card-body py-0">
                                <!--  صندوق الأفكار -->
                                <div class="d-flex flex-column">
                                    <div class="card bg-transparent border-0 py-0">
                                        <div class="card-body py-0 my-0">
                                            <div class="row mx-0 px-0 my-0 py-0">
                                                <div class="col-lg-3 col-md-6 col-12 p-4">
                                                    <h6 class="fw-bold mb-0">
                                                        مطلوب مشروع سياحى
                                                    </h6>
                                                </div>
                                                <div class="col-lg-4 col-md-6 col-12 p-4 border_custom_idea">
                                                    <h6 class="fw-bold mb-2">
                                                        رأس المال المعروض =
                                                        500,000 ريال
                                                    </h6>
                                                    <h6 class="fw-bold mb-0">
                                                        مرغوب فى تنفيذه فى :
                                                        دول الخليج، أو مصر
                                                    </h6>
                                                </div>
                                                <div class="col-lg-5 col-md-6 col-12 p-4">
                                                    <h6 class="fw-bold mb-0 line-height-1">
                                                        تتوفر الموارد التالية: شركة قائمة، موظفون متخصصون، آليات
                                                        ومعدات
                                                    </h6>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-12">
                        <div class="card border-0 shadow-sm rounded-8 position-relative">
                            <!-- index -->
                            <div class="bg-custom position-absolute top-0 end-0 bg_idea_num rounded-circle m-1">
                                <div class="d-flex align-items-center justify-content-center h-100">
                                    <span class="text-white fw-bold fs-6">
                                        2
                                    </span>
                                </div>
                            </div>
                            <div class="card-body py-0">
                                <!--  صندوق الأفكار -->
                                <div class="d-flex flex-column">
                                    <div class="card bg-transparent border-0 py-0">
                                        <div class="card-body py-0 my-0">
                                            <div class="row mx-0 px-0 my-0 py-0">
                                                <div class="col-lg-3 col-md-6 col-12 p-4">
                                                    <h6 class="fw-bold mb-0">
                                                        مطلوب مشروع سياحى
                                                    </h6>
                                                </div>
                                                <div class="col-lg-4 col-md-6 col-12 p-4 border_custom_idea">
                                                    <h6 class="fw-bold mb-2">
                                                        رأس المال المعروض =
                                                        500,000 ريال
                                                    </h6>
                                                    <h6 class="fw-bold mb-0">
                                                        مرغوب فى تنفيذه فى :
                                                        دول الخليج، أو مصر
                                                    </h6>
                                                </div>
                                                <div class="col-lg-5 col-md-6 col-12 p-4">
                                                    <h6 class="fw-bold mb-0 line-height-1">
                                                        تتوفر الموارد التالية: شركة قائمة، موظفون متخصصون، آليات
                                                        ومعدات
                                                    </h6>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-12">
                        <div class="card border-0 shadow-sm rounded-8 position-relative">
                            <!-- index -->
                            <div class="bg-custom position-absolute top-0 end-0 bg_idea_num rounded-circle m-1">
                                <div class="d-flex align-items-center justify-content-center h-100">
                                    <span class="text-white fw-bold fs-6">
                                        3
                                    </span>
                                </div>
                            </div>
                            <div class="card-body py-0">
                                <!--  صندوق الأفكار -->
                                <div class="d-flex flex-column">
                                    <div class="card bg-transparent border-0 py-0">
                                        <div class="card-body py-0 my-0">
                                            <div class="row mx-0 px-0 my-0 py-0">
                                                <div class="col-lg-3 col-md-6 col-12 p-4">
                                                    <h6 class="fw-bold mb-0">
                                                        مطلوب مشروع سياحى
                                                    </h6>
                                                </div>
                                                <div class="col-lg-4 col-md-6 col-12 p-4 border_custom_idea">
                                                    <h6 class="fw-bold mb-2">
                                                        رأس المال المعروض =
                                                        500,000 ريال
                                                    </h6>
                                                    <h6 class="fw-bold mb-0">
                                                        مرغوب فى تنفيذه فى :
                                                        دول الخليج، أو مصر
                                                    </h6>
                                                </div>
                                                <div class="col-lg-5 col-md-6 col-12 p-4">
                                                    <h6 class="fw-bold mb-0 line-height-1">
                                                        تتوفر الموارد التالية: شركة قائمة، موظفون متخصصون، آليات
                                                        ومعدات
                                                    </h6>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-12">
                        <div class="card border-0 shadow-sm rounded-8 position-relative">
                            <!-- index -->
                            <div class="bg-custom position-absolute top-0 end-0 bg_idea_num rounded-circle m-1">
                                <div class="d-flex align-items-center justify-content-center h-100">
                                    <span class="text-white fw-bold fs-6">
                                        4
                                    </span>
                                </div>
                            </div>
                            <div class="card-body py-0">
                                <!--  صندوق الأفكار -->
                                <div class="d-flex flex-column">
                                    <div class="card bg-transparent border-0 py-0">
                                        <div class="card-body py-0 my-0">
                                            <div class="row mx-0 px-0 my-0 py-0">
                                                <div class="col-lg-3 col-md-6 col-12 p-4">
                                                    <h6 class="fw-bold mb-0">
                                                        مطلوب مشروع سياحى
                                                    </h6>
                                                </div>
                                                <div class="col-lg-4 col-md-6 col-12 p-4 border_custom_idea">
                                                    <h6 class="fw-bold mb-2">
                                                        رأس المال المعروض =
                                                        500,000 ريال
                                                    </h6>
                                                    <h6 class="fw-bold mb-0">
                                                        مرغوب فى تنفيذه فى :
                                                        دول الخليج، أو مصر
                                                    </h6>
                                                </div>
                                                <div class="col-lg-5 col-md-6 col-12 p-4">
                                                    <h6 class="fw-bold mb-0 line-height-1">
                                                        تتوفر الموارد التالية: شركة قائمة، موظفون متخصصون، آليات
                                                        ومعدات
                                                    </h6>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="d-flex align-items-center gap-2 justify-content-center my-4">
                    <button type="button" class="btn btn-custom py-3 px-4">
                        <span class="small fw-bold">
                            عرض المزيد
                        </span>
                    </button>
                </div>
            </div>
        </div>
    </div>
</div>
