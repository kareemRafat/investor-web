        ...
        return $table
            ...
            ->columns([
                ...
                TextColumn::make('profit_type')
                    ...
                    ->badge()
                    ->getStateUsing(
                        fn ($record) => optional($record->profits->first())->profit_type
                    )
                    ->colors([
                        'success' => 'annual',
                        'info' => 'one-time',
                    ...
                    ->formatStateUsing(
                        fn ($state) => $state === 'annual' ? 'سنوية' : 'مرة واحدة'
                    ),
                TextColumn::make('cost_type')
                    ->label('التكاليف')
                    ->badge()
                    ->getStateUsing(
                        fn ($record) => optional($record->costs->first())->cost_type
                    )
                    ->colors([
                        'danger' => 'annual',
                        'warning' => 'one-time',
            ...
            ->recordActions([
                ...
                ViewAction::make()
                    ->url(
                        fn ($record): string => IdeaResource::getUrl('view', ['record' => $record])
                    ),
            ])
            ->toolbarActions([
